...
% 
% Für die Erstellung des Dokuments wurde eine Reihe an LaTeX Paketen verwendet. Diese sind unter der LPPL (http://www.latex-project.org/lppl.txt) Lizenz zu finden. 
%
...
\usepackage{german} %Deutsche Sprache
\usepackage[]{graphicx} %Zusatzfunktionen für Graphiken, remove draft for release
\usepackage{blindtext} %für lorem ipsum blindtext
\usepackage{multicol} %für zweispaltig etc.
\usepackage[a4paper]{geometry} %Abstände innerhalb der Seiten
\geometry{
        a4paper,
        top=32mm,
        outer=13mm, 
...
\usepackage{float} %Float Optionen für Graphiken
%Header ------
\usepackage{fancyhdr} %Paket zum Einstellen des Headers
\fancyhead{}
\fancyfoot{}
...
%----- Farben
\usepackage{anyfontsize} %Größere/Kleinere Zeichen
\usepackage[table]{xcolor} %Farben definierne können, table für rowcolor
...
\usepackage{sectsty} %Kontrollieren von Chapter, Section Positon
...
\usepackage{authoraftertitle}%title, author und date einstellen und immer abrufen können

%--- boxes
\usepackage{tcolorbox} %Farbige Box
\usepackage{tikz} %Tikz, um zu zeichnen
\tcbuselibrary{skins}
%---- bessere auzählung
\usepackage{enumitem}
...
\usepackage{background}%Hintergründe ienstellen
\backgroundsetup{
        scale=1,
        opacity=0.6,
        angle=0,
